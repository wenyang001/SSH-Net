...
import lpips
def batch_LPIPS(img, imclean):
    # 确保图像在 [0, 1] 范围内，并且是 torch.Tensor 类型
    img = img.clamp(0, 1).cpu()
    imclean = imclean.clamp(0, 1).cpu()

    # 创建 LPIPS 损失函数（默认使用 AlexNet）
    loss_fn = lpips.LPIPS(net='vgg', verbose=False).cpu()

    LPIPS_score = 0
    for i in range(img.shape[0]):
        # 计算单张图像的 LPIPS 值
        lpips_value = loss_fn(img[i:i+1], imclean[i:i+1])
        LPIPS_score += lpips_value.item()

    # 返回批次平均值
    return LPIPS_score / img.shape[0]
...
def add_watermark_noise_test(img_train, occupancy=50, img_id=3, scale_img=1.5, self_surpervision=False,
                                same_random=0, alpha=0.3,location=128):
    # 加载水印,水印应该是随机加入
    # random_img = random.randint(1, 13)
    # 对比实验的时候选取某个水印进行去除
...
def load_froze_vgg16(device_ids):
    # finetunning
